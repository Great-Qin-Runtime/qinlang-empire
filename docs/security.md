...
完整字段定义、默认值、当前实施程度（V0.3 软约束 → V1.0 硬约束）见 **[`security-permissions.md`](security-permissions.md)**。
...
**未声明 permissions 字段** 不是错误（不阻断 PR）但会在每次 dispatch 注入 `W0601 missing-permissions` warn 事件，提醒贡献者明示。

## 5. 命令白名单

`tools/validate_all.py` 会拒绝以下模式：
...
调度器在写 `empire/state.json` 与 `empire/history.jsonl` 前自动脱敏：
